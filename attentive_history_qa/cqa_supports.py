...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.history_qa_ids = history_qa_ids
...
def read_quac_examples(input_file, is_training):
    ...
    for entry in input_data:#一个entry是一篇问答
        ...
        for i, (question, answer, followup, yesno) in enumerate(zip(questions, answers, followups, yesnos)):
            ...
            history_answer_marker = None
            #new modify, 让history_answer_marker仍然为None，但是保留history_turns和history_turns_text
            if FLAGS.use_history_answer_marker:
                ...
                for history_turn, (each_answer, each_question) in enumerate(
                    ...
                    metadata['history_turns_text'].append((each_question[0], each_answer[0])) #[(q1, a1), (q2, a2), ..., (qk-1, ak-1)]
            elif FLAGS.use_new_attetion:
                start_index = 0
                for history_turn, (each_answer, each_question) in enumerate(
                    zip(answers[start_index: end_index], questions[start_index: end_index])):
            ...
            #如果不进上面的else,那么现在的question_with_histories只存储了当前问题，没有历史信息与当前回答
            question_with_histories += question[0]
            #每个qas中的qa代表一个问题，它的历史对话存储在metadata中
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
                                 doc_stride, max_query_length, max_ans_length, threshold, is_training):
    """Loads a data file into a list of `InputBatch`s."""
    
    unique_id = 1000000000

    features = []
    max_history_length = (max_query_length) + (max_ans_length)
    # max_history_length = (max_query_length) * 3 + (max_ans_length) * 2 - 2
    ...
    for (example_index, example) in enumerate(examples):
        ...
        #new modify: the history question:
        if len(metadata['tok_history_answer_markers']) == 0:
            history_tokens = query_tokens
        else:
            history_tokens, ans = metadata['history_turns_text'][0][0], metadata['history_turns_text'][0][1]
            history_tokens = tokenizer.tokenize(history_tokens)
            ans = tokenizer.tokenize(ans)
            if len(history_tokens) >= max_query_length:
                history_tokens = history_tokens[0:max_query_length]
            if len(ans) >= max_ans_length:
                ans = ans[0:max_ans_length]
            history_tokens += ans

        if len(history_tokens) > max_history_length - 2:
            history_tokens = history_tokens[:max_history_length-2]
        ...
        #doc_tokens存储了paragraph_text的所有tokens
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            token_is_max_context = {}
            segment_ids = []
            qa_tokens.append("[CLS]")
            tokens.append("[CLS]")
            marker.append(0)
            segment_ids.append(0)
            for token in history_tokens:
                qa_tokens.append(token)
            ...
            history_ids = tokenizer.convert_tokens_to_ids(qa_tokens)
            ...
            if FLAGS.front_padding:
                ...
                while len(input_ids) < max_seq_length:
                    ...
                    segment_ids.append(0)
                    marker.append(0)
                while len(history_ids) < max_history_length:
            ...
            assert len(history_ids) == max_history_length
            assert len(history_mask) == max_history_length
...
#                         "answer: %s" % (tokenization.printable_text(answer_text)))
            ...
            features.append(
                InputFeatures(
                    ...
                    tokens=tokens,
                    history_qa=history_tokens,
                    token_to_orig_map=token_to_orig_map,
                    token_is_max_context=token_is_max_context,
                    input_ids=input_ids,
                    history_qa_ids = history_ids,
                    ...
                    metadata=metadata))
            unique_id += 1
            # print("history_qa_ids: ", features[0].history_qa_ids)
    return features
...
def convert_examples_to_variations_and_then_features(examples, tokenizer, max_seq_length, 
                                doc_stride, max_query_length, max_ans_length, threshold, max_considered_history_turns, is_training):
    ...
    # 注意：examples = train_examples = read_quac_examples(input_file=FLAGS.quac_train_file, is_training=True)
    ...
    if is_training:
        np.random.seed(0)
        idx = np.random.permutation(num_examples)#制作了一个shuffle的idx矩阵
        examples_shuffled = np.asarray(examples)[idx]
    else:
        examples_shuffled = np.asarray(examples)
    
    #example = {'id': question[1], 'question': question_with_histories, 'answers': [{'answer_start': answer[1], 'text': answer[0]}],
    #                    'history_answer_attention': history_answer_attention, 'metadata': metadata}
    #examples是所有文章，example是单个文章
    for example_index, example in enumerate(examples_shuffled):
        ...
        #variations是单个文章中所有问题，variation是单个问题，features是该问题的所有变体
        for variation_index, variation in enumerate(variations):
            features = convert_examples_to_features([variation], tokenizer, max_seq_length, doc_stride, max_query_length, max_ans_length, threshold, is_training)
    ...
    for example in examples:
        ...
        if len(example.metadata['tok_history_answer_markers']) == 0:
            ...
            for history_turn, marker, history_turn_text in zip(
                ...
                #这一段是得到example variation:"question + passage + markers (M3)"，所以要把markers删掉哈
                #这里的each_new_example.metadata['tok_history_answer_markers'] = [marker]全是0
...
def convert_features_to_feed_dict(features):
    batch_unique_ids, batch_input_ids, batch_history_qa_ids, batch_input_mask, batch_history_mask = [], [], [], [], []
    ...
    for feature in features:
        ...
        batch_history_qa_ids.append(feature.history_qa_ids)
        batch_input_mask.append(feature.input_mask)
        batch_history_mask.append(feature.history_mask)
    ...
    feed_dict = {'unique_ids': batch_unique_ids, 'input_ids': batch_input_ids, 'history_qa_ids': batch_history_qa_ids,
              'input_mask': batch_input_mask, 'history_mask': batch_history_mask, 'segment_ids': batch_segment_ids, 
