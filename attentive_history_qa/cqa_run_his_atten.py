...
if FLAGS.do_train:
    ...
    try:
        ...
        train_features, example_tracker, variation_tracker, example_features_nums = convert_examples_to_variations_and_then_features(
                                                            ...
                                                            max_ans_length= FLAGS.max_answer_length,
                                                            threshold=FLAGS.max_answer_threshold,
                                                            max_considered_history_turns=FLAGS.max_considered_history_turns, 
                                                            is_training=True)
        path = FLAGS.cache_dir + FLAGS.dataset.lower()
        if not os.path.exists(path):
            if os.path.exists(FLAGS.cache_dir):
                os.mkdir(path)
            else:
...
max_history_length = (FLAGS.max_query_length) + (FLAGS.max_answer_length)
# max_history_length = (FLAGS.max_query_length) * 3 + (FLAGS.max_answer_length) * 2 + 2
...
history_qa_ids = tf.placeholder(tf.int32, shape=[None, max_history_length], name='history_qa_ids')
input_mask = tf.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name='input_mask')
history_mask = tf.placeholder(tf.int32, shape=[None, max_history_length], name='history_mask')
...
bert_representation, cls_representation, weights = bert_rep(
        ...
        for step, batch in enumerate(train_batches):
            ...
            try:
                _, train_summary, total_loss_res = sess.run([train_op, merged_summary_op, total_loss], 
                                                feed_dict={unique_ids: fd['unique_ids'], input_ids: fd['input_ids'], history_qa_ids: fd['history_qa_ids'],
                                                input_mask: fd['input_mask'], history_mask: fd['history_mask'], segment_ids: fd['segment_ids'], 
