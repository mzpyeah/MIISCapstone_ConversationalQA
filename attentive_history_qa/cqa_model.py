...
# def Embedding(bert_config, is_training, input_ids, history_qa_ids, segment_ids, history_answer_marker, use_one_hot_embeddings):
#     model = modeling.Embedding(
#         config=bert_config,
...
#     embedding, history_output = model.get_embedding_output()
    
#     return embedding, history_output

# def bert_rep(bert_config, is_training, input_ids, input_tensor, input_mask):
#     model = modeling.Bert(
#         config=bert_config,
#         is_training=is_training,
#         input_ids=input_ids,
...
def bert_rep(bert_config, is_training, input_ids, history_qa_ids, input_mask, history_mask, segment_ids, history_answer_marker, use_one_hot_embeddings):
    ...
    return final_hidden, sent_rep, scores
...
def new_attention(input_tensor, history_output, history_mask, x3 = None):
 
    # shape for input_tensor:   ? * 12 * 384 * 768
    # shape for history_qa_ids: ? * 12 * 114 * 768
    # shape for history_mask:   ? * 12 * 114

    # batch = input_tensor.shape[0]
    # word_num1 = input_tensor.shape[1]
    # word_num2 = history_qa_ids.shape[1]

    if x3 is None:
        x3 = history_output

    input_tensor = tf.expand_dims(input_tensor, 1)
    input_tensor = tf.tile(input_tensor, [1, history_output.shape[1], 1, 1])#? * 12 * 384 * 768

    # scores: batch x word_num1 x word_num2
    scores = scoring(input_tensor, history_output)#? * 12 * 384 * 114

    # empty_mask = history_mask.eq(0).unsqueeze(1).expand_as(scores)
    history_mask = tf.expand_dims(history_mask, 2)#12，12，1，114
    empty_mask = tf.tile(history_mask, [1, 1, scores.shape[2], 1])#矩阵扩张 12，12，384，114
    ...
    # history_mask = tf.cast(history_mask, tf.bool)
    # history_mask = tf.equal(history_mask, False)
    # history_mask = tf.expand_dims(history_mask, 1)
    # empty_mask = tf.tile(history_mask, [1, score.shape[1], 1])#矩阵扩张
    ...
    # return new_bert_representationtf.squeeze(probs)
